...
> **Note:** Beta version has different syntax, but is fully operational.
...
```cpp
GEMItem menuItemInt("Number:", &number, GEM_VAL_INTEGER);
GEMItem menuItemBool("Enable print:", &enablePrint, GEM_VAL_BOOLEAN);
```

> Note the use of `&` operator (to pass pointer to a variables) and predefined `GEM_VAL_INTEGER` and `GEM_VAL_BOOLEAN` aliases, that explicitly determine the type of associated with menu item variable. The other available values (`GEM_VAL_BYTE`, `GEM_VAL_CHAR`, `GEM_VAL_SELECT`) should be used when creating a menu item of the corresponding type.

Create menu button that will trigger `printData()` function. It will print value of our `number` variable to Serial monitor if `enablePrint` is `true`. We will write (define) this function later. However we should forward-declare it in order to pass it to `GEMItem` constructor. Let's name our button "Print":
...
> **Note:** `GEM` constructor supports additional optional parameters that can customize look of the menu. See Reference and [wiki](https://github.com/Spirik/GEM/wiki) for details.
...
Full version of this basic example is shipped with the library and can be found at "examples/Example-01_Basic/[Example-01_Basic.ino](https://github.com/Spirik/GEM/blob/beta/examples/Example-01_Basic/Example-01_Basic.ino)".
...
* **setSplash(** _uint8_t PROGMEM_ *sprite **)**  
  *Returns*: nothing  
  ...
  For more details and example, see [wiki](https://github.com/Spirik/GEM/wiki).
...
* **clearContext()**  
  *Returns*: nothing  
  Clear context. Assigns `nullptr` values to function pointers of the `context` property.
...
Menu item of the menu. Can represent editable or read-only variable of type `int`, `byte`, `boolean`, `char[17]`; option select of type `int`, `byte`, `char*`; link to another menu page; or button that can invoke user-defined actions and create action-specific context, which can have its own enter (setup) and exit callbacks as well as loop function. User-defined callback function can be specified to invoke when editable menu item is saved or option is selected. Exact definition of `GEMItem` object depends on its type.

#### Variable

```cpp
GEMItem menuItemVar(title, &linkedVariable, linkedType[, readonly]);
```
or
```cpp
GEMItem menuItemVar(title, &linkedVariable, linkedType[, saveCallback]);
```
...
* **&linkedVariable**  
  *Type*: `pointer`  
  Pointer to variable that menu item is associated with.

* **linkedType**  
  *Type*: `byte`  
  *Values*: `GEM_VAL_INTEGER`, `GEM_VAL_BYTE`, `GEM_VAL_CHAR`, `GEM_VAL_BOOLEAN`  
  Defines type of the associated variable: `int`, `byte`, `char[17]`, or `boolean` accordingly.
...
```cpp
GEMItem menuItemSelect(title, &linkedVariable, linkedType, select[, readonly]);
```
or
```cpp
GEMItem menuItemSelect(title, &linkedVariable, linkedType, select[, saveCallback]);
```
...
* **linkedType**  
  ...
  *Values*: `GEM_VAL_SELECT`  
  In case of option select should be set to `GEM_VAL_SELECT`.

* **select**  
  *Type*: `GEMSelect*`  
  Pointer to [`GEMSelect`](#gemselect) option select object that represents a list of available values.
...
* **GEM_VAL_INTEGER**  
  ...
  *Value*: `0`  
  Alias for supported types of associated with menu item variable (submitted as **linkedType** setting to `GEMItem` constructor of variable menu items): associated variable is of type `int`.

* **GEM_VAL_BYTE**  
  *Type*: macro `#define GEM_VAL_BYTE 1`  
  *Value*: `1`  
  Alias for supported types of associated with menu item variable (submitted as **linkedType** setting to `GEMItem` constructor of variable menu items): associated variable is of type `byte`.

* **GEM_VAL_CHAR**  
  *Type*: macro `#define GEM_VAL_CHAR 2`  
  *Value*: `2`  
  Alias for supported types of associated with menu item variable (submitted as **linkedType** setting to `GEMItem` constructor of variable menu items): associated variable is of type `char[17]`.

* **GEM_VAL_BOOLEAN**  
  *Type*: macro `#define GEM_VAL_BOOLEAN 3`  
  *Value*: `3`  
  Alias for supported types of associated with menu item variable (submitted as **linkedType** setting to `GEMItem` constructor of variable menu items): associated variable is of type `boolean`.

* **GEM_VAL_SELECT**  
  *Type*: macro `#define GEM_VAL_SELECT 4`  
  *Value*: `4`  
  Alias for supported types of associated with menu item variable (submitted as **linkedType** setting to `GEMItem` constructor of option select menu item): associated variable is either of type `int`, `byte` or `char[]` with option select used to pick a predefined value from the list of available options (note that in case of `char[]` variable, character array should be big enough to hold select option with the longest value).

* **GEM_STR_LEN**  
  *Type*: macro `#define GEM_STR_LEN 17`  
  *Value*: `17`  
  Alias for supported length of the string (character sequence) variable of type `char[GEM_STR_LEN]`.
...
```cpp
GEMSelect mySelect(type, length, optionsArray);
```

* **type**  
  *Type*: `byte`  
  *Values*: `GEM_VAL_INTEGER`, `GEM_VAL_BYTE`, `GEM_VAL_CHAR`  
  Type of the options of the select.

* **length**  
  *Type*: `byte`  
  Length of `optionsArray`. Should be explicitly supplied because array is passed by reference. Easy way to provide array length is to calculate it using the following expression: `sizeof(optionsArray)/sizeof(SelectOptionInt)`, or `sizeof(optionsArray)/sizeof(SelectOptionByte)`, or `sizeof(optionsArray)/sizeof(SelectOptionChar)` depending of the type of the array used.
...
GEMSelect mySelect(GEM_VAL_INTEGER, sizeof(optionsArray)/sizeof(SelectOptionInt), optionsArray);
```
or
```cpp
// Integer option select
...
GEMSelect mySelect(GEM_VAL_INTEGER, 3, (SelectOptionInt[]){{"Opt 1", 10}, {"Opt 2", -12}, {"Opt 3", 13}});
```
...
* **val_char**  
  *Type*: `char*`  
  Value of the option that is assigned to linked variable upon option selection. Note that character array of associated with menu item variable (of type `char[]`) should be big enough to hold select option with the longest value.
